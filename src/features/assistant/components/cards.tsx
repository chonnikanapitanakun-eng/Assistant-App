...
export function CardView({ card, onConfirm, onDismiss, onEdit }: { card: Card; onConfirm: (c: ProposalCard) => void; onDismiss: (c: ProposalCard) => void; onEdit?: (c: ProposalCard, proposal: Proposal) => void }) {
  if (card.type === 'list') return <ListCard title={card.title} rows={card.rows} />;
  if (card.type === 'stats') return <StatsCard rows={card.rows} />;
  if (card.proposal.kind === 'apply_plan') return <PlanView card={card} plan={card.proposal} onConfirm={() => onConfirm(card)} onDismiss={() => onDismiss(card)} onEdit={(p) => onEdit?.(card, p)} />;
 ...
 */
function PlanView({ card, plan, onConfirm, onDismiss, onEdit }: { card: ProposalCard; plan: PlanProposal; onConfirm: () => void; onDismiss: () => void; onEdit: (p: PlanProposal) => void }) {
  const { t, i18n } = useTranslation();
  const { colors, tints, spacing, radius } = useTheme();
      ...
      <View>
        {plan.slots.map((s, i) => (
          <View key={s.taskId} style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, minHeight: 52, paddingLeft: spacing.md, paddingRight: spacing.xs, borderTopWidth: 1, borderTopColor: colors.border, borderBottomWidth: i === plan.slots.length - 1 ? 1 : 0, borderBottomColor: colors.border }}>
            ...
            {pending ? <IconButton icon="x" label={t('assistant.plan.remove', { title: s.title })} onPress={() => remove(s.taskId)} /> : null}
          </View>
        ))}
      </View>
      ...
      <View style={{ paddingHorizontal: spacing.md }}>
        {pending ? (
          <View style={{ flexDirection: 'row', gap: spacing.sm, flexWrap: 'wrap' }}>
            <Button size="sm" icon="check" label={t('assistant.plan.approve', { count: plan.slots.length })} onPress={onConfirm} disabled={!plan.slots.length} />
