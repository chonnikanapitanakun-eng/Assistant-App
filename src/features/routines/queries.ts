...
import { routinesDue, routineTaskId, taskFromRoutine } from './model';
...
async function generate(date: string): Promise<number> {
  ...
  const rows = due.map((r) => {
    ...
    return { ...task, id: routineTaskId(r.id, date), createdAt: t, updatedAt: t, reminderAt: combineDateTime(date, task.startTime ?? undefined) ?? null };
  });
